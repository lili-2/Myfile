...
function getData(a, b, c = '') {
    ...
    let city_result = saveCity.some(item => item.data_city.trim() === b.trim() && item.data_province.trim() === a.trim())
...
function update(data) {
    ...
    let max = Math.max.apply(Math, maxDegree)
    let min = Math.min.apply(Math, minDegree)
...
map(arr,fn)
arr.mymap = (arr,fn,n = 0) => {
    const [head,...tail] = arr
    return (arr.length > 0) ? [fn(head,n + 1), ...map(tail,fn,n+1)] : 
}
// const map = (array, fn, n = 0) => {
//     const [head, ...tail] = array;
//     return (array.length > 0) ? [fn(head, n + 1), ...map(tail, fn, n + 1)] : [];
// }
