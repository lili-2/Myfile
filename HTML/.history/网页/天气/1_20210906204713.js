...
function update(data) {
    ...
    for (let i = 0; i <= 23; i++) {
        ...
        if (parseInt(hourDatatime.slice(8, 10)) - parseInt(srTime) === 1) {
            hours_innerHTML(hoursListEle,data.rise[0].sunrise,rise,日出)
            // hoursListEle.innerHTML +=   
            //                             `<li class="hour_data">
        ...
        if ((parseInt(hourDatatime.slice(8, 10) - parseInt(ssTime))) === 1) {
            hours_innerHTML(hoursListEle,data.data.rise[0].sunset,set,日落)
        ...
        hours_innerHTML(hoursListEle,`${hourDatatime.slice(8, 10)}:${hourDatatime.slice(11, 13)}`,set,${data.forecast_1h[i].degree}°`)
        // hoursListEle.innerHTML +=   `<li class="hour_data">
...
function hours_innerHTML(dom,time,pic,text){
    dom.innerHTML +=
    // night/${data.forecast_1h[i].weather_code}
        `<li class="hour_data">
            <p class="hour_data_time">${data.rise[0].sunrise}</p>
