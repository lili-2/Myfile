...
guanzhu.addEventListener('click', () => {
    ...
    if (guanzhu.innerHTML === `[添加关注]`) {
        saveCity = (JSON.parse(localStorage.getItem('saveCity')) || [])
        // item.attributes['data-province']
        console.log(pos_data.attributes['data-province'].value)
        saveCity = [{ data_province: pos_data.attributes['data-province'].value, data_city: pos_data.attributes['data-city'].value, data_county: pos_data.attributes['data-county'].value }]
        localStorage.setItem(`savecity`, JSON.stringify(historys))
    }
...
function getData(a, b, c = '') {
    ...
    // let updategz = saveCity.every(item => {
