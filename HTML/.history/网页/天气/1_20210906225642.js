...
function saveCity(value) {
    ...
    concern.push({
        ...
        data_county: value.getAttribute('data_county')
    })
...
hotCity.forEach(function (i) {
    i.addEventListener('mousedown', function () {
        ...
        //     pos.innerHTML = `<p data-province = ${this.attributes['data-province'].value} data-city=${this.attributes['data-city'].value}} class="pos_data">${this.attributes['data-city'].value}</p>`
        // } else {
        //     pos.innerHTML = `<p data-province = ${this.attributes['data-province'].value} data-city=${this.attributes['data-city'].value}} class="pos_data">${this.attributes['data-province'].value} ${this.attributes['data-city'].value}</p>`
        // }
        cityInput.style.display = 'none'
...
function choose_click() {
    ...
    p.forEach((item, index) => {
        item.addEventListener('mousedown', () => {
            ...
            // pos.innerHTML = `<p data-province = ${this.attributes['data-province'].value} data-city=${this.attributes['data-city'].value}} class="pos_data">${this.attributes['data-province'].value} ${this.attributes['data-city'].value}</p>`
            // pos.innerHTML = `<p data-province = ${this.getAttribute(data-province)} data-city=${this.getAttribute(data-city)} data-county="" class="pos_data">${this.getAttribute(data-province)} ${this.getAttribute(data-city)}</p>`
            // getData()
            if (cityStr.length === 2) {
                ...
                pos.innerHTML = `<p data-province = ${cityStr[0]} data-city=${cityStr[1]}} class="pos_data">${cityStr[0]} ${cityStr[1]}</p>`
                ...
                // pos.setAttribute('data_county', cityStr[2])
...
//             // addScriptTag(`https://wis.qq.com/weather/common?source=pc&weather_type=observe|forecast_1h|forecast_24h|index|alarm|limit|tips|rise&province=${item.data_province}&city=${item.data_city}&county=${item.data_county}&callback=concerns`);
...
function judgeCity(dom) {
    if (dom.attributes['data-province'].value === dom.attributes['data-city'].value) {
        pos.innerHTML = `<p data-province = ${dom.attributes['data-province'].value} data-city=${this.attributes['data-city'].value}} class="pos_data">${dom.attributes['data-city'].value}</p>`
    } else {
        pos.innerHTML = `<p data-province = ${this.attributes['data-province'].value} data-city=${this.attributes['data-city'].value}} class="pos_data">${dom.attributes['data-province'].value} ${this.attributes['data-city'].value}</p>`
    }
}