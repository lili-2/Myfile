...
guanzhu.addEventListener('click', () => {
    console.log
    const pos1_data = document.querySelector('#pos>pos_data')
    console.log(pos1_data)
    const pos_data = guanzhu.getElementsByClassName('pos_data')
    if (guanzhu.innerHTML === `[添加关注]`) {
        console.log(pos1_data)
        saveCity(pos_data)
    }
    else guanzhu.innerHTML = `[已关注]`
})
//保存到saveCity中
