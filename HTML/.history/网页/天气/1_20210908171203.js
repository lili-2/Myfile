...
guanzhu.addEventListener('click', () => {
    ...
    if (guanzhu.innerHTML === `[添加关注]`) {
        saveCity = (JSON.parse(localStorage.getItem('savecity')) || [])
        saveCity = [sa{ data_province: pos_data.attributes['data-province'].value, data_city: pos_data.attributes['data-city'].value, data_county: pos_data.attributes['data-county'].value}]
