...
function update(data) {
    ...
    draw(arrDegree,'rgb(252,192,112)',16)
    function draw(maxDegree,minDegree,cirColor,height){
        for (let i = 0; i <= 7; i++) {
            drawcir(unitX + 90 * i, (arrDegree[i] - max) * unitY * (-1) + 30, 'rgb(252,192,112)')
            if (i === 0) {
                dirText(`19px`, `${arrDegree[i]}°`, (unitX + 90 * i - 10), (maxDegree[i] - max) * unitY * (-1) + height, "rgb(194,194,194)")
            } else {
                dirText(`19px`, `${arrDegree[i]}°`, (unitX + 90 * i - 10), (maxDegree[i] - max) * unitY * (-1) + height, "rgb(56,76,120)")
            }
            if (i < 7) {
