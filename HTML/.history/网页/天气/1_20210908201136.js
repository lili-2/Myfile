...
function update(data) {
    ...
    draw(maxDegree,'rgb(252,192,112)',16)
    draw(minDegree,'rgb(252,192,112)',16)
    function draw(arrDegree,cirColor,height){
        for (let i = 0; i <= 7; i++) {
            ...
            if (i < 7) {
                drawline(unitX + 90 * i, (arrDegree[i] - max) * unitY * (-1) + 30, unitX + 90 * (i + 1), (arrDegree[i + 1] - max) * unitY * (-1) + 30, 'rgb(252,192,112)')
            }
        }
    }
    // for (let i = 0; i <= 7; i++) {
