...
function getData(a, b, c = '') {
    ...
    if (updategz) { guanzhu.innerHTML = `[添加关注]`
    } else {
        guanzhu.innerHTML = `[已关注]`    }
...
function update(data) {
    ...
    const today_data = [`中央气象台${dayTime.slice(8, 10)}:${dayTime.slice(10, 12)}发布`,`${data.observe.degree}°`,`${data.observe.weather}`,`${winds_arr[data.observe.wind_direction]} ${data.observe.wind_power}级`,`湿度 ${data.observe.humidity}%`,]
    const dayTime = `${data.observe.update_time}`
    data_today[0].innerHTML = 
    data_today[1].innerHTML = 
    data_today[2].innerHTML = 
    data_today[3].innerHTML = 
    data_today[4].innerHTML = 
    data_today[5].innerHTML = 
