...
function update(data) {
    ...
    judgeDay(`sunrise`,data)
    function judgeDay(time,data){
        let time = data.rise[0].time
        if ((parseInt(myDate.getHours()) - parseInt(time.slice(0, 2))) <= 1) {
            srTime = data.rise[0].time
        } else {
            srTime = data.rise[1].time
        }
    }
