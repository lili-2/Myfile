...
function update(data) {
    ...
    const maxDegree = []
    const minDegree = []
    console.log(Object.values(data.forecast_24h).map((item) => {return parseInt(item.max_degree)}))
    Math.max.apply(Math,[1,2,3,4,6,8,2])
    console.log(Math.max(Object.values(data.forecast_24h).map((item) => {return parseInt(item.max_degree)})))
