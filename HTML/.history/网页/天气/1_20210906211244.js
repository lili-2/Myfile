...
function update(data) {
    ...
    for (let i = 0; i <= 7; i++) {
        days_innerhtml(i,dats7_top,data.forecast_24h[i].time,data.forecast_24h[i].day_weather,data.forecast_24h[i].day_weather_code)
        // dats7_top.innerHTML += `<div class="days_top">
        //                             <p class="days_top_time1">昨天</p>
        ...
        // } else {
        //     days_top_time1.innerHTML = `${getMyDay(new Date(time))}`
        // }
...
function days_innerhtml(i,dom,time,weather,day_weather_code){
    console.log(time)
    let time1 = time
    if(i === 0){
        time1 = `昨天`
...
// </div> `
        ...
        // days_innerhtml(i,dats7_top,data.forecast_24h[i],data.forecast_24h[i].day_weather,data.forecast_24h[i].day_weather_code)
    dom.innerHTML += 
        `<div class="days_top">
            <p class="days_top_time1">${time1}</p>
            <p class="days_top_time2">${time.slice(5, 7)}月${time.slice(8, 10)}日</p>
            <p class="days_top_tianqi">${weather}</p>
            <div class="days_top_pic" style="background-image: url(https://mat1.gtimg.com/pingjs/ext2020/weather/pc/icon/weather/day/${day_weather_code}.png);"></div>
        </div> `
}