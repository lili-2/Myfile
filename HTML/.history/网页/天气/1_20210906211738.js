...
function update(data) {
    ...
    for (let i = 0; i <= 7; i++) {
        daystop_innerhtml(i,dats7_top,data.forecast_24h[i].time,data.forecast_24h[i].day_weather,data.forecast_24h[i].day_weather_code)
    }
    ...
    for (let i = 0; i <= 7; i++) {
        ...
        dats7_bottom.innerHTML = ` <div class="days_footer">
                            <div class="days_footer_pic"></div>
                            <p class="days_footer_tianqi">大雨</p>
                            <p class="days_footer_wind">西南风 3级</p>
                        </div>`
...
function daystop_innerhtml(i,dom,time,weather,day_weather_code){
    let time1 = time
    if(i === 0){ time1 = `昨天`
    }else if(i === 1){ time1 = `今天`
    }else{ time1 = `${getMyDay(new Date(time))}` }
    dom.innerHTML += 
        `<div class="days_top">
