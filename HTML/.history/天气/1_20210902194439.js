window.onload = function () {
    ...
    function update(data) {
        ...
        const dats7_bottom = document.getElementsByClassName("dats7_top")[0]
        const middle_daysbottom = document.createDocumentFragment()
        
        for (let i = 0; i <= 7; i++) {
            const days_bottom = document.createElement('div')
            days_bottom.classList.add('days_bottom')

            const days_bottom_time1 = document.createElement('p')
            const days_bottom_time2 = document.createElement('p')
            const days_bottom_tianqi = document.createElement('p')
            const days_bottom_pic = document.createElement('div')

            days_bottom_time1.classList.add('days_top_time1')
            days_bottom_time2.classList.add('days_top_time2')
            days_bottom_tianqi.classList.add('days_top_tianqi')
            days_bottom_pic.classList.add('days_top_pic')

            days_bottom.appendChild(days_bottom_time1)
            days_bottom.appendChild(days_bottom_time2)
            days_bottom.appendChild(days_top_tianqi)
            days_bottom.appendChild(days_top_pic)
            middle_daystop.appendChild(days_top)
