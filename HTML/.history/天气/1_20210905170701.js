...
hotCity.forEach(function (i) {
    i.addEventListener('mousedown', function () {
        if (this.attributes['data-province'].value === this.attributes['data-city'].value) {
            ...
            pos.setAttribute('data-province',this.attributes['data-city'].value)
            pos.setAttribute('data-city',this.attributes['data-city'].value)
        } else {
...
function getData(a, b, c = '') {
    ...
    for (let i = 0; i < historys.length; i++) {
        his.innerHTML += `<li class="top_right" data-province=${historys[i].data-province} data-city=${historys[i].data-city} data-county=${historys[i].data-county})>${historys[i].data-city}</li>`
    }
...
function update(data) {
    //当天数据更新
    const data-today = document.getElementsByClassName("data-today")
    ...
    data-today[0].innerHTML = `中央气象台${dayTime.slice(8, 10)}:${dayTime.slice(10, 12)}发布`
    data-today[1].innerHTML = `${data.observe.degree}°`
    data-today[2].innerHTML = `${data.observe.weather}`
    data-today[3].innerHTML = `${winds_arr[data.observe.wind_direction]} ${data.observe.wind_power}级`//wind_direction
    data-today[4].innerHTML = `湿度 ${data.observe.humidity}%`
    data-today[5].innerHTML = `气压 ${data.observe.pressure}hPa`
    ...
    if (`${data.limit.tail_number}` !== '不限行') {
        data-today[6].innerHTML = `限行 ${data.limit.tail_number}`
    } else {
        const car_pic = document.getElementById('car_pic')
        data-today[6].style.display = 'none'
        car_pic.style.display = 'none'
    }

    //逐小时天气预报
    const hour_data-time = document.getElementsByClassName("hour_data-time")
    ...
    hoursListEle.classList.add('hour_data-list')
    ...
    for (let i = 0; i <= 23; i++) {
        ...
        if (parseInt(hourDatatime.slice(8, 10)) - parseInt(srTime) === 1) {
            hoursListEle.innerHTML += `<li class="hour_data">
                                                <p class="hour_data-time">${data.rise[0].sunrise}</p>
                                                <div class="hour_data-pic" style="background-image: url(https://mat1.gtimg.com/pingjs/ext2020/weather/pc/icon/weather/rise.png)"></div>
                                                <p class="hour_data-degree">日出</p>
                                            </li>`
        }
        if ((parseInt(hourDatatime.slice(8, 10) - parseInt(ssTime))) === 1) {
            hoursListEle.innerHTML += `<li class="hour_data">
                                            <p class="hour_data-time">${data.rise[0].sunset}</p>
                                            <div class="hour_data-pic" style="background-image: url(https://mat1.gtimg.com/pingjs/ext2020/weather/pc/icon/weather/set.png)"></div>
                                            <p class="hour_data-degree">日落</p>
                                        </li>`
        }

        hoursListEle.innerHTML += `<li class="hour_data">
                                                <p class="hour_data-time">${hourDatatime.slice(8, 10)}:${hourDatatime.slice(11, 13)}</p>
                                                <div class="hour_data-pic" style="background-image: url(https://mat1.gtimg.com/pingjs/ext2020/weather/pc/icon/weather/night/${data.forecast_1h[i].weather_code}.png)"></div>
                                                <p class="hour_data-degree">${data.forecast_1h[i].degree}°</p>
                                            </li>`
    ...
    for (let i = 0; i <= 7; i++) {
        ...
        dirText(`19px`, `${maxDegree[i]}°`, (unitX + 90 * i - 10), (maxDegree[i] - max) * unitY * (-1) + 16)
        if (i < 7) {
    ...
    for (let i = 0; i <= 7; i++) {
        ...
        dirText(`19px`, `${minDegree[i]}°`, (unitX + 90 * i - 10), (minDegree[i] - max) * unitY * (-1) + 58)
        if (i < 7) {
    ...
    p.forEach((e, index) => {
        e.addEventListener('mousedown', () => {
            ...
            if (cityStr.length === 2) {
                ...
                pos.setAttribute('data-province',cityStr[0])
                pos.setAttribute('data-city',cityStr[0])
            } else if (cityStr.length === 3) {
                getData(cityStr[0], cityStr[1], cityStr[2])
                pos.innerHTML = `${cityStr[1]} ${cityStr[2]}`
                pos.setAttribute('data-province',cityStr[0])
                pos.setAttribute('data-city',cityStr[1])
                pos.setAttribute('data-district',cityStr[2])
            }
        })
    })
}
...
//             addScriptTag(`https://wis.qq.com/weather/common?source=pc&weather_type=observe|forecast_1h|forecast_24h|index|alarm|limit|tips|rise&province=${item.data-province}&city=${item.data-city}&county=${item.data-county}&callback=concerns`);
