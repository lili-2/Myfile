window.onload = function () {
    ...
    if (typeof(storage) !== "undefined") {
        storage.setItem(`saveCity`, `[]`);
    } else {
        console.log("Sorry, your browser does not support Web Storage...");
    }

    guanzhu.addEventListener('click',()=>{
        saveCity("city",pos.innerHTML)
    })
    
    function saveCity(name,value){
        // for(var i=0;i<storage.length;i++){
        //     var key=storage.key(i);
        // }
        var data = storage.getItem("saveCity");
        arr = JSON.parse(data);
        // console.log(arr)
        arr.add({
         : `${name}`
        })
            // localStorage.setItem(`${name}${storage.length}`, value);
