window.onload = (function(document){
    ...
    function update(data){
        ...
        //逐小时天气预报
        const hour_data_time = documentument.getElementsByClassName("hour_data_time")
        const hours_data= documentument.getElementsByClassName('hours_data')[0]
        
        //forecast_1h
        const hoursListEle = documentument.createElement('ul')
        hoursListEle.classList.add('hour_data_list')
        for(let i = 1;i <= 24;i++){
            const hourEle = documentument.createElement('li')
            const hourDataTime = documentument.createElement('p')
            const hourDataPic = documentument.createElement('div')
            const hourDataDegree = documentument.createElement('p')
    ...
    const btn_r1 = documentument.getElementsByClassName('btn_r')[0]
    console.log(btn_r1)
