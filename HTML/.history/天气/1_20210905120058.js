...
function update(data) {
    ...
    for (let i = -2; i <= 23; i++) {
        if(i === -2){
            ...
            if((myDate.getHours() > sunrise.slice(0,2)) <= 1){
                srTime = data.rise[0].sunset
            }else{
                srTime = data.rise[1].sunset
            }
        }
        else {
            let hourDatatime = data.forecast_1h[i].update_time
            if((hourDatatime.slice(8, 10) - srTime) <= 1){
                hoursListEle.innerHTML += `<li class="hour_data">
                                                <p class="hour_data_time">${data.rise[0].sunrise}:${hourDatatime.slice(11, 13)}</p>
            ...
            }else if((hourDatatime.slice(8, 10) - ssTime) <= 1){
                hoursListEle.innerHTML += `<li class="hour_data">
