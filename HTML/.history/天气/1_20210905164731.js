...
function updataCon(data) {
    ...
    if (concern.length === 0) { cityConcern.innerHTML = `<li id="city_concern">点击“添加关注”添加城市哟~</li>` }
        ...
        concern.forEach(item => {
            addScriptTag(`https://wis.qq.com/weather/common?source=pc&weather_type=observe|forecast_1h|forecast_24h|index|alarm|limit|tips|rise&province=${item.data_province}&city=${item.data_}&county=${c}&callback=concerns`);
