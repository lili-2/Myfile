window.onload = function () {
    ...
    function update(data) {
        ...
        // // drawcir(100,20)
        // for (let i = 0; i <= 7; i++) {
        //     drawcir(unitX + 90*i,(maxDegree[i] - max)*unitY*(-1) + 20,'red')
        //     dirText(`20px`,`${maxDegree[i]}°`,(unitX + 90*i - 10),(maxDegree[i] - max)*unitY*(-1) + 15)
        // }
        // for(let i = 0;i <=7 ;i++){
        //     drawcir(unitX + 90*i,(minDegree[i] - max)*unitY*(-1) + 20,'blue')
        //     dirText(`20px`,`${minDegree[i]}°`,(unitX + 90*i - 10),(minDegree[i] - max)*unitY*(-1) + 15)
        // }
        const unitY = parseInt((200 - 60) / (max - min))
        console.log(unitY)
        // const unitX = 30, 125, 220]
        const unitX = 30
        ...
        for (let i = 0; i <= 7; i++) {
            ...
            if(i < 7){
                drawline(unitX + 90 * i,(maxDegree[i] - max) * unitY * (-1) + 30,unitX + 90 * (i+1) - 10,(maxDegree[i+1] - max) * unitY * (-1) + 60,'blue')
            }
        }
    }
    //获取星期几
