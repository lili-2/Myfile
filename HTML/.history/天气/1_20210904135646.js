window.onload = function () {
    ...
    function update(data) {
        ...
        // drawcir(100,20)
        for (let i = 0; i <= 7; i++) {
            drawcir(unitX + 90*i,(maxDegree[i] - max)*unitY*(-1) + 20)
            dirText(`20px`,`${maxDegree[i]}°`,(unitX + 95*i - 10),(maxDegree[i] - max)*unitY*(-1) + 15)
        }
        // drawcir(unitX[2],(maxDegree[0] - max)*unitY*(-1) + 20)
        // dirText(`20px`,`${maxDegree[0]}°`,(unitX[2] - 10),(maxDegree[0] - max)*unitY*(-1))
    }
    //获取星期几
