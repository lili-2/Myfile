window.onload = function () {
    ...
    function getData(a, b, c = '') {
        ...
        if(history.length === 0){ saveHistory(history,a,b,c);console.log('b')}
        console.log(history)
        history.forEach((item) => {
            console.log(item.data_province === a && item.data_city === b && item.data_county === c)
            if(item.data_province === a && item.data_city === b && item.data_county === c){ return;
            }else{saveHistory(history,a,b,c),console.log('a')}
        })
    ...
    function updataHis(){
        ...
        history.forEach((item)=>{
            ...
            hor_item.setAttribute('data-province',item.data_province)
            hor_item.setAttribute('data-city',item.data_city)
            hor_item.setAttribute('data-county',item.data_county)
            hor_item.innerHTML = `${item.data_city}`
