...
function getData(a, b, c = '') {
    ...
    if (historys.length === 0) { saveHistory(historys, a, b, c); }
    let flag = true;
    ...
    addScriptTag('https://wis.qq.com/weather/common?source=pc&weather_type=observe|forecast_1h|forecast_24h|index|alarm|limit|tips|rise&province=陕西省&city=西安市&county=&callback=cb');
}
//搜索绑定事件 回车搜索出所有城市
...
function update(data) {
    ...
    for (let i = -2; i <= 23; i++) {
        ...
        else {hoursListEle.innerHTML += `<li class="hour_data">
                                        <p class="hour_data_time">
                                        ${hourDatatime.slice(8, 10)}:${hourDatatime.slice(11, 13)}
                                        </p>
                                        <div class="hour_data_pic" style="background-image: url(https://mat1.gtimg.com/pingjs/ext2020/weather/pc/icon/weather/night/${data.forecast_1h[i].weather_code}.png)">
                                            
                                        </div>
                                        <p class="hour_data_degree">
                                            ${data.forecast_1h[i].degree}°
                                        </p>
                                    </li>`
        // if (i === -2) {
...
function updataHis() {
    ...
    if (historys.length >= 1) {
        for (let i = historys.length - 1; i >= 0; i--) {
            ...
            hor_item.setAttribute('data-province', historys[i].data_province)
            hor_item.setAttribute('data-city', historys[i].data_city)
            hor_item.setAttribute('data-county', historys[i].data_county)
            hor_item.innerHTML = `${historys[i].data_city}`
...
//history保存在本地
function saveHistory(historys, a, b, c) {
    historys.push({
        data_province: a,
        data_city: b,
        data_county: c
