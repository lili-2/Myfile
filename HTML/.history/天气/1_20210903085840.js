window.onload = function () {
    ...
    function update(data) {
        ...
        for (let i = 0; i <= 7; i++) {
            ...
            const time = `data.forecast_24h[i]`
            const time = `${data.forecast_24h[i].time.slice(5, 7)}-${data.forecast_24h[i].time.slice(8, 10)}`
            console.log(time)
            days_top_time1.innerHTML = `${getMyDay(new Date("2019-05-16"))}`
