window.onload = (function(doc){
    ...
    function update(data){
        ...
        //逐小时天气预报
        const hour_data_time = doc.getElementsByClassName("hour_data_time")
        const hours_data= doc.getElementsByClassName('hours_data')[0]
        
        //forecast_1h
        const hoursListEle = doc.createElement('ul')
        hoursListEle.classList.add('hour_data_list')
        for(let i = 1;i <= 24;i++){
            const hourEle = doc.createElement('li')
            const hourDataTime = doc.createElement('p')
            const hourDataPic = doc.createElement('div')
            const hourDataDegree = doc.createElement('p')
    ...
    const btn_l1 = doc.getElementsByClassName('btn_l')[0]
    const btn_r1 = doc.getElementsByClassName('btn_r')[0]
    console.log(btn_r1)
