...
hotCity.forEach(function (i) {
    i.addEventListener('mousedown', function () {
        if (this.attributes['data_province'].value === this.attributes['data_city'].value) {
            pos.innerHTML = `${this.attributes['data_city'].value}`
            pos.setAttribute('data_province',this.attributes['data_city'].value)
            pos.setAttribute('data_city',this.attributes['data_city'].value)
        } else {
            pos.innerHTML = `${this.attributes['data_province'].value} ${this.attributes['data_city'].value}`
            pos.setAttribute('data_province',this.attributes['data_province'].value)
            pos.setAttribute('data_city',this.attributes['data_city'].value)
        }
        cityInput.style.display = 'none'

        getData(this.attributes['data_province'].value, this.attributes['data_city'].value)
    })
})
...
function getData(a, b, c = '') {
    ...
    for (let i = 0; i < historys.length; i++) {
        his.innerHTML += `<li class="top_right" data_province=${historys[i].data_province} data_city=${historys[i].data_city} data_county=${historys[i].data_county})>${historys[i].data_city}</li>`
    }
...
//             cityConcern.innerHTML += `<li class="itemcity" data_province="陕西省" data_city="西安市" data_district="">
