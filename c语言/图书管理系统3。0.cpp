...
void amend_book()    //修改图书信息 
{
	do
	{
		...
		printf("--------------------------------------------------------------------------------------------");
		toxy(20,9);
		printf("登录号    书名        作者名         图书类别            出版时间          价格        数量");
		toxy(20,10);
		printf("-----------------------------------------------------------------------------------------");
		while(p!=NULL)
		{
			if(p!=NULL&&strcmp(p->name,_name)==0)
			{
				toxy(20,i);
				j++;
				printf("%10s%10s%10s%10s%8s%10s%8.2f%10d\n",j,p->number,p->name,p->author,p->type,p->time,p->price,p->num);
				p->x=j;    //给符合查询标准的结点标号 
				i++;
			}
			p=p->next;
...
void find_name_book()  //按名字查询 
{
	...
	while(p!=NULL)
	{
		...
		{
			toxy(20,i);
			printf("%10s%10s%10s%10s%8s%8.2f%10d\n",p->number,p->name,p->author,p->type,p->time,p->price,p->num);
			i++;
		}
		p=p->next;
	}
...
void fuzzy_search()   //模糊查询 
{
	...
	while(p!=NULL)
	{
		if(p!=NULL&&(strcmp(p->name,information)==0||strcmp(p->author,information)==0||strcmp(p->number,information))==0)
		{
			toxy(20,i);
			printf("%10s%10s%10s%10s%8s%8.2f%10d\n",p->number,p->name,p->author,p->type,p->time,p->price,p->num);
			i++;
